...
| 07 | [Analytics](./features/07-analytics.md) | 🔴 · 🔴 · 🟡 | 🟢 | **W1** — 5 audit bugs (today UTC vs IST midnight, 3rd broken `computeRosterStatus`, SLA divisor mismatch, zero-completed→0-not-100, NaN guard on date) + 4 arch fixes (shared `_helpers.ts`, aggregation push-down on agents, single roster source). **W2** — new `/api/analytics/breakdown` endpoint + tabbed AnalyticsTabs (Sources / Rules / Stores / Task Types). **W3** — Trends tab with inline-SVG SLA / completion / breach time-series. **W4** — Cohorts tab (agents grouped by hire-month). **W5** — transversal data-source slicer at top of page; every panel scoped. | CSV / chart export; volume forecast; anomaly detection; funnel analytics; CSAT integration — all P1/P2 follow-ups. |
...
| 9 | `computeRosterStatus` uses local Node TZ via `toTimeString` | `src/lib/roster/availability.ts:57` | ⚠️ Partial — analytics' broken third copy was deleted (Analytics W1); the canonical `lib/roster/availability.ts` still has the issue but is now the single source consumed by everyone |
