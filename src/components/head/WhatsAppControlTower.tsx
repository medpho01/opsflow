...
type Conversation = {
  ...
  unread: number; openTickets: number;
...
export function WhatsAppControlTower() {
  ...
  // Instant updates via SSE; refetch on every server ping. Safety poll as fallback.
  useEffect(() => {
    ...
    es.onmessage = (e) => {
      try {
        ...
        if (d.changed) { loadConvos(); if (activeRef.current) loadDetail(activeRef.current); }
      } catch { /* keepalive */ }
    };
    es.onerror = () => setLive(false); // browser auto-reconnects
    const poll = setInterval(() => {
      loadConvos(); if (activeRef.current) loadDetail(activeRef.current);
  ...
  function openConvo(c: Conversation) {
    if (!c.ticketId) { flash("No actionable ticket in this group yet"); return; }
    setActiveId(c.ticketId);
    setConvos((cs) => cs.map((x) => (x.groupId === c.groupId ? { ...x, unread: 0 } : x))); // optimistic
  }
  ...
  async function sendReply() {
    ...
    setReply(""); loadConvos(); loadDetail(activeId);
  }
  async function setStatus(status: string) {
    ...
    flash(`Marked ${status.replace("_", " ").toLowerCase()}`); loadConvos(); loadDetail(activeId);
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-5 py-3 border-b border-zinc-800">
        ...
        {live && <span className="text-[11px] text-emerald-400/80" title="Live updates via SSE">● live</span>}
      ...
      <div className="grid grid-cols-[340px_1fr_320px] flex-1 min-h-0">
        ...
        <div className="border-r border-zinc-800 flex flex-col min-h-0">
            ...
            <div className="flex gap-1.5">
              {(["ALL", "UNREAD", "ACTION"] as const).map((f) => (
                <button key={f} onClick={() => setFilter(f)}
                  className={`text-xs font-medium px-2.5 py-1 rounded-full border ${filter === f ? "bg-blue-600 border-blue-600 text-white" : "border-zinc-700 text-zinc-400 hover:border-zinc-500"}`}>
          ...
          <div className="overflow-y-auto flex-1">
            ...
            {shown.map((c) => (
              <button key={c.groupId} onClick={() => openConvo(c)}
                className={`w-full text-left px-3 py-2.5 border-b border-zinc-800/60 flex gap-3 ${detail?.group?.id === c.groupId ? "bg-zinc-800/60" : "hover:bg-zinc-900"}`}>
                ...
                <span className="flex-1 min-w-0">
                  ...
                  <span className="flex items-center gap-2 mt-0.5">
                    <span className={`text-xs truncate ${c.unread ? "text-zinc-200" : "text-zinc-500"}`}>
                  ...
                  <span className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                    {c.topIntent && <span className={`text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded border ${intentChip(c.topIntent)}`}>{c.topIntent.replace("_", " ")}</span>}
                    {c.topOrderId && <span className="text-[11px] font-mono text-zinc-500">#{c.topOrderId}</span>}
                    ...
                    {c.openTickets > 1 && <span className="text-[10px] text-zinc-500">{c.openTickets} open</span>}
                  </span>
                </span>
              </button>
            ))}
        ...
        <div className="flex flex-col min-h-0 border-r border-zinc-800">
          {!detail ? (
            <div className="flex-1 grid place-items-center text-zinc-600 text-sm">Select a conversation</div>
          ) : (
            <>
              <div className="px-4 py-3 border-b border-zinc-800 flex items-center gap-2">
                <span className="font-semibold text-zinc-100">{short(detail.group?.subject || "")}</span>
            ...
            <>
              ...
              <div className="p-4 border-b border-zinc-800 flex flex-col gap-2">
                ...
                <button onClick={() => { setTarget("lab"); setReply(`Team, need help on #${detail.ticket.orderId || detail.ticket.requestId} — please confirm.`); }}
                  className="text-left text-sm border border-zinc-700 hover:border-blue-500 rounded-lg px-3 py-2 text-zinc-200">→ Ask the lab <span className="block text-xs text-zinc-500">{detail.labGroup ? short(detail.labGroup.subject) : "no lab group linked"}</span></button>
              </div>
